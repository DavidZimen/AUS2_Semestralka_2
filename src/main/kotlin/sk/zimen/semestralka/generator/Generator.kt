...
class Generator() {
    ...
    fun generateTestItems(count: Int): List<TestItem> {
        val items: MutableList<TestItem> = ArrayList(count)
        val keys: MutableSet<Long> = HashSet(count)
        while (keys.size < count) {
            keys.add(random.nextLong())
        }
        keys.forEach {
            items.add(TestItem(it, nextString(TestItem.MAX_STRING_LENGTH)))
        }
    ...
    fun <T : Place> generateItems(
        ...
        val items = java.util.ArrayList<T>(count)
